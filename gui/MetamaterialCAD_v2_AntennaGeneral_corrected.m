function MetamaterialCAD_v2_AntennaGeneral()
    ...
    overlayMetamaterialCheck = uicheckbox(antennaActionPanel, ...
        'Text','Show metamaterial', ...
        'Value',true, ...
        'Position',[15 40 150 22], ...
        'ValueChangedFcn',@(~,~) renderAntennaPreview());
    ...
    % ============================================================
    % TOGGLE GLOBAL ARRAY MODE
    % ============================================================
    function toggleArray()
        % Array dimensions are taken from the Nx/Ny/dx/dy controls in
        % the Antenna tab. This callback keeps the legacy Structure
        % dropdown from referencing a missing function.
        updatePlot();
    end
    ...
    function params = collectParams()
        ...
        % Antenna overlay array configuration
        ...
        % Keep the existing global Array mode functional as well.
        % These aliases are consumed by the existing buildArray calls.
        params.Nx = params.antennaNx;
        params.Ny = params.antennaNy;
        params.dx = params.antennaDx;
        params.dy = params.antennaDy;
        ...
        if ~isempty(geom)
            ...
            try
                ...
                if strcmp(params.type,'ssrr')
                    ...
                    % Gielis: use the configured physical ring spacing
                    % as a stable first-guess gap value. The previous code
                    % referenced params.gap, which was never defined.
                    gap_m = max(params.spacing,0.01) * 1e-3;
                end
    ...
    function reloadAntennaDXF()
        ...
        % Use the unit information detected directly from the DXF.
        % The previous version referenced dxfUnitsDrop, but that control
        % did not exist in the GUI.
        scaleToMM = autoScale;
        unitName = autoUnit;
    ...
    function fitMetamaterialToAntenna()
        ...
        try
            ...
            if antennaArrayEnableCheck.Value && ...
