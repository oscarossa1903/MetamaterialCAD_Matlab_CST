function MetamaterialCAD_v2_AntennaGeneral()
    ...
    % AXES
    % ============================================================
    % Each preview lives in its own container panel. Hiding the
    % panel hides everything drawn inside it (hiding only the axes
    % would leave its lines/patches visible on top of the other view).
    metaViewPanel = uipanel(fig, ...
        'Position',[465 180 900 690], ...
        'BorderType','none', ...
        'BackgroundColor',[0.965 0.97 0.98]);

    ax = uiaxes(metaViewPanel, ...
        'Position',[0 0 900 690], ...
        'Box','on', ...
        'FontName','Segoe UI', ...
        'FontSize',11);
    ...
    % Copper thickness along Z (0 = flat sheet, as before)
    metalThicknessSpinner = createSpinner( ...
        sharedPanel, ...
        'Metal Thickness (mm)', ...
        0.035, ...
        [0 1], ...
        0.005, ...
        86);
    metalThicknessSpinner.Tooltip = ...
        ['Copper thickness in Z. Standard PCB: 0.035 mm (1 oz) ' ...
         'or 0.018 mm (0.5 oz). 0 exports flat sheets.'];
    ...
    % ------------------------------------------------------------
    antennaViewPanel = uipanel(fig, ...
        'Position',[465 180 900 690], ...
        'BorderType','none', ...
        'BackgroundColor',[0.965 0.97 0.98], ...
        'Visible','off');

    antennaAxes = uiaxes(antennaViewPanel, ...
        'Position',[0 0 900 690], ...
        'Box','on', ...
        'FontName','Segoe UI', ...
        'FontSize',11);
    ...
    function toggleArray()
        ...
        % Array dimensions (Nx, Ny, dx, dy) come from the
        % "Metamaterial Array" panel in the Antenna tab.
        updatePlot();
    end
    ...
    function params = collectParams()
        ...
        % Copper thickness along Z (mm). 0 = zero-thickness sheet.
        params.metalThickness = metalThicknessSpinner.Value;
        ...
        % ARRAY PARAMETERS
        % ========================================================
        % Structure = Array uses the same controls as the
        % "Metamaterial Array" panel in the Antenna tab.
        params.Nx = round(antennaNxSpinner.Value);
        params.Ny = round(antennaNySpinner.Value);
        params.dx = antennaDxSpinner.Value;
        params.dy = antennaDySpinner.Value;
    ...
    function updatePlot()
        ...
        % Keep the antenna preview synchronized with every design
        % change, even while it is hidden in another tab.
        ...
        % SUBSTRATE
        % ========================================================
        % In Array mode the board covers the whole matrix
        [boardLx,boardLy] = boardExtent(params);
            ...
            try
                ...
                try
                    ...
                    fQuick = universal_rapid_guess( ...
                        ...
                        max(params.metalThickness,1e-3) * 1e-3, ...
    ...
    % ============================================================
    % BOARD EXTENT (substrate / CSRR plate)
    % Single cell: substrateSize x substrateSize
    % Array:       covers the full Nx x Ny matrix
    % ============================================================
    function [Lx,Ly] = boardExtent(p)

        Lx = p.substrateSize;
        Ly = p.substrateSize;

        if isfield(p,'structure') && strcmp(p.structure,'Array')
            Lx = (p.Nx-1)*p.dx + p.substrateSize;
            Ly = (p.Ny-1)*p.dy + p.substrateSize;
        end
    end
    ...
    function switchMainWorkspace()
        ...
        if isAntenna
            metaViewPanel.Visible = 'off';
            antennaViewPanel.Visible = 'on';
            renderAntennaPreview();
        else
            antennaViewPanel.Visible = 'off';
            metaViewPanel.Visible = 'on';
            updatePlot();
        end
    end
    ...
    function applyDXFCalibrationAndRefresh()
        ...
        if rawWidth <= eps && rawHeight <= eps
            error('The DXF has no drawable extent (zero size).');
        end

        % ========================================================
        % REAL DXF DIMENSIONS (1:1)
        %
        % Dimensions are taken directly from the DXF, converted to
        % millimeters with the unit factor ($INSUNITS or the manual
        % "DXF Units" selection). No width is forced.
        % ========================================================
        antennaDXFCalibrationScale = 1.0;
    ...
    function updateInspector(params)
        ...
        if params.metalThickness > 0
            summaryConductor.Text = sprintf('%s, t = %.3f mm', ...
                params.conductorMaterial, params.metalThickness);
        else
            summaryConductor.Text = sprintf('%s, flat sheet', ...
