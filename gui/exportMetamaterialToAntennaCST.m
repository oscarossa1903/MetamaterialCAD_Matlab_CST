...
% To remove one export in CST, delete its component.
    ...
    %
    % params.metalThickness (mm):
    %   > 0 : real solids from z = antennaZ to antennaZ + t
    ...
    for k = 1:nShapes
        ...
        if isSolid
            ...
            % ----------------------------------------------------
            % SOLID CONDUCTOR WITH REAL THICKNESS
            % ----------------------------------------------------
            outerCmd = buildCSTExtrude( ...
                outerSheet, component, params.conductorMaterial, ...
                g.outerX, g.outerY, z, t);

            invoke(mws,'AddToHistory', ...
                ['Outer Solid ',historyTag],outerCmd);

            if hasInner
                innerCmd = buildCSTExtrude( ...
                    innerSheet, component, 'Vacuum', ...
                    g.innerX, g.innerY, z, t);

                invoke(mws,'AddToHistory', ...
                    ['Inner Solid ',historyTag],innerCmd);
            end
            ...
            outerCurveCmd = buildCSTCurve( ...
                g.outerX, g.outerY, z, outerCurve, curveFolder);

            invoke(mws,'AddToHistory', ...
                ['Outer Curve ',historyTag],outerCurveCmd);

            outerSheetCmd = buildCSTSheet( ...
                outerCurve, outerSheet, params.conductorMaterial, ...
                component, curveFolder);

            invoke(mws,'AddToHistory', ...
                ['Outer Sheet ',historyTag],outerSheetCmd);

            if hasInner
                innerCurveCmd = buildCSTCurve( ...
                    g.innerX, g.innerY, z, innerCurve, curveFolder);

                invoke(mws,'AddToHistory', ...
                    ['Inner Curve ',historyTag],innerCurveCmd);
                ...
                invoke(mws,'AddToHistory', ...
                    ['Inner Sheet ',historyTag],innerSheetCmd);
            end
        end
        ...
        if hasInner
            subtractCmd = buildCSTSubtract( ...
                outerSheet, innerSheet, component);

            invoke(mws,'AddToHistory', ...
                ['Subtract ',historyTag],subtractCmd);
        end
    end
    ...
    fprintf(['Metamaterial conductor inserted into antenna CST project.\n' ...
             '  Component: %s  |  shapes: %d  |  skipped: %d  |  t = %.3f mm\n'], ...
             component, nShapes - nSkipped, nSkipped, t);
end
